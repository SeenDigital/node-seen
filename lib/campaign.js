...
Campaign.prototype.entries = function (status, params, next) {
  ...
  api.get(url, null, function (data) {
    ...
    var twitter = createPayload(self.campaignId, ids.twitter);


    async.parallel([fetchMedia('instagram', instagram)], function () {
      next(data);
    });

    /*
    api.post('instagam/media', instagram, function(medias) {
