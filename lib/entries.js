...
var Entries = function (id, type, params, textTweets) {
  var self = this;
  var url = 'campaigns/' + id;

  Api.get(url, null, function (campaign) {
    campaignHandler(self, type, campaign, params, textTweets);
  });
...
var campaignHandler = function (self, type, campaign, params, textTweets) {
  var baseUrl = 'campaigns/' + campaign.id + '/entries';
  var url = filterType(type, baseUrl);

  Api.get(url, params, function (entries) {
    entriesHandler(self, campaign, entries, textTweets);
  });
};
...
var entriesHandler = function (self, campaign, entries, textTweets) {
    ...
    for ( var i = 0; i < entries.data.length; i++ ) {
     ...
     var twitter = createPayload('twitter', campaignId, cache.twitter);
      ...
      fetchMediaFormatted(instagram, entries, function (media) {
        fetchTweetsFormatted(twitter, entries, textTweets, function (tweets) {
          var data = {
...
var fetchTweetsFormatted = function (payload, entries, textTweets, next) {
