...
var Entries = function (id, type, params, next) {
  var self = this;
...
var campaignHandler = function (self, type, campaign, params, next) {
  ...
  var url = filterType(type, baseUrl);
    ...
    console.log('params.descending '+params.descending);
  Api.get(url, params, function (entries) {
    entriesHandler(self, campaign, entries, next);
  });
};
...
var entriesHandler = function (self, campaign, entries, next) {
     ...
     var twitter = createPayload('twitter', campaignId, cache.twitter);
      ...
      fetchMediaFormatted(instagram, entries, function (media) {
        fetchTweetsFormatted(twitter, entries, function (tweets) {
          var allMedia = media.concat(tweets);
          allMedia.sort(function(thisGallery, thatGallery){
               if (thisGallery.timestamp > thatGallery.timestamp) return -1;
               if (thisGallery.timestamp < thatGallery.timestamp) return 1;
               return 0;
          });
  ...
  tmp.username = entry.username;
    ...
    if ( entry.platform === 'instagram' ) {
        ...
        if ( entry.type === 'photo' ) {
            ...
            tmp.mediaUrl = media.videos?media.videos.standard_resolution.url:media.images.standard_resolution.url;
        }
